...
ITERATE over the array from i = 0 to i = length - 1
  ITERATE over the array from j = 0 to j = length - 2 - i
...
This step is simple. You only need to return `left + 1` as the new `index` for `quicksort`. This should happen after your `while` loop.
